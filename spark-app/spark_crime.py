...
def process_batch(batch_df, batch_id):
    ...
    if sent_count >= 900:
        ...
        print(f"⏳ Moins de 900 lignes disponibles à envoyer à l'IA ({sent_count})")

def start_flux_crime(spark):
    
    # Streaming depuis Kafka
    kafka_read_crime_df = spark.readStream \
        .format("kafka") \
        .option("kafka.bootstrap.servers", "kafka:9092") \
    ...
    query_crime = kafka_read_crime_df.writeStream \
